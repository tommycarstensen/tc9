...
def main():
    ...
    l_bfiles, l_strands, l_miss, l_multiple = init()

    ## 0) update sex
    ## i.e. run PLINK --update-sex
    l_bfiles[0] = PLINK_update_sex(l_bfiles[0])


    ## 5) For the Octo and quad strand files - ensure both are on build 37
    ## *before* deduplication
    l_strands[0] = update_quad_strand_to_build37(l_strands[0])

    ## 6)  exclude SNPs in miss/multiple files
    ## *before* deduplication
    l_strands = exclude_strand_miss_and_multiple(l_strands,l_miss,l_multiple,)

    ## 7) exclude chr+pos duplicates within strand files
    ## *before* join
    ## *after* liftOver
    l_strands = exclude_strand_duplicates(l_strands)

    ## 8) join strand files on chr+pos+allele
    ## *after* deduplication
    l_strands_venn = list(l_strands)
    l_strands = exclude_strand_noncommon(l_strands)


    ## 1a) flip bim files according to build 36 and build 37 strand files
    ## 1b) remove selected samples
    ## *before* updating from build 36 to build 37
    l_bfiles = PLINK_flip(l_bfiles,l_strands,)

    ## 2) update bim positions from build 36 to build 37
    ## *after* flipping according to build 36 strand file
    ## or skip and join bim and strand on rsID instead
    l_bfiles[0] = PLINK_update_map(l_bfiles[0])

    ## 3) exclude SNPs in miss/multiple files
    ## *before* deduplication
    l_bfiles = PLINK_exclude_miss_and_multiple(l_bfiles,l_miss,l_multiple,)

    ## 4) exclude duplicates *after* updating from build 36 to build 37
    ## or skip and join bim and strand on rsID instead
    l_bfiles = PLINK_exclude_duplicates(l_bfiles)


    ## 9a) find SNPs common between bim files and "harmonised"/"biblical" strand file
    ## 9b) and extract
    l_bfiles_venn = list(l_bfiles)
    l_bfiles = PLINK_extract_intersection(l_strands,l_bfiles)

    ## 10) rename quad SNPs (update rsIDs from build 36 to build 37)
    ## i.e. run PLINK --update-map --update-name
    l_bfiles[0] = PLINK_rename(l_bfiles[0])

    ## 11) split by population (previously QC_agv_preQC_split_populations.py)
    ## i.e. run PLINK --keep
    PLINK_split_by_population(l_bfiles)

    ## 12) merge selected quad and octo samples (previously QC_agv_preQC_merge_chips.py)
    ## i.e. run PLINK --bmerge
    PLINK_bmerge(l_bfiles)

    ## 13) count SNPs at each step
    count_SNPs()

    ## 14) create 4 set Venn
...
def exclude_strand_miss_and_multiple(l_strands_in,l_miss,l_multiple,):
    ...
    for i in xrange(2):
        ...
        l_strands_out += [strand_out]
        if os.path.isfile('%s.strand' %(strand_out)):
            if (
                os.path.getsize('%s.strand' %(strand_out)) > 0
                and
                time.time()-os.path.getmtime('%s.strand' %(strand_out))<hours*60*60
                ):
                continue
...
def PLINK_extract_intersection(l_strands,l_bfiles):
    ...
    identify_common_strand_bim(l_strands,l_bfiles,)
...
def identify_common_strand_bim(l_strands,l_bfiles,):
    ...
    for fn in [
        ...
        for setname,condition in [
            ...
            print cmd
            print setname, int(os.popen(cmd).read())
    ...
    ##
    ## 7) additional exclusion of allele mismatches (i.e. kgp22784170)
    ##
##    for i in xrange(2):
##        bfile = l_bfiles[i]
...
##        cmd += 'print $%i > "%s.common.SNPs"' %(3*i+1,bfile,)
    cmd += 'print $1,$4 > "common.SNPs.pasted"'
    cmd += "}'"
    execmd(cmd)
...
##    kgp22784170
##    join -1 7 -2 1 -o 1.2,1.5,1.6 omni2.5-4_20120904_agv_gtu_sexupdated_flipped_build37_exclmissmult_deduplicated.bim.sorted bim.strand.common.positions  > tmp4
##    join -1 7 -2 1 -o 1.2,1.5,1.6 omni2.5-8_agv_20120910_gtu_flipped_exclmissmult_deduplicated.bim.sorted bim.strand.common.positions  > tmp8
##    paste tmp4 tmp8 | awk '{if($2!=0&&$5!=0&&!(($2==$5&&$3==$6)||($2==$6&&$3==$5))) print}'
##    paste tmp4 tmp8 | awk '{if(!(($3==$6)||($3==$5)||($2==$6))) print}' | wc -l
##        paste tmp4 tmp8 | awk '{if(!(($2==0&&$3==0)||($5==0&&$6==0))&&!(($3==$6)||($3==$5)||($2==$6))) print}'
##        paste tmp4 tmp8 | awk '{if(!(($2==$5&&$3==$6)||($2==$6&&$3==$5))&&!(!(($2==0&&$3==0)||($5==0&&$6==0))&&!(($3==$6)||($3==$5)||($2==$6)))) print}'
...
def exclude_strand_duplicates(l_strands_in):
    ...
    for strand_in in l_strands_in:
        ...
        strand_out = strand_in+'_deduplicated'
        l_strands_out += [strand_out]
        if os.path.isfile('%s.strand' %(strand_out)):
            if (
                os.path.getsize('%s.strand' %(strand_out)) > 0
...
def PLINK_flip(l_bfiles,l_strands,):
    ...
    for i in xrange(2):
        ...
        fn_extract = 'join.%s.%s.SNPs' %(fn_strand,bfile,)
        cmd = 'join -1 1 -2 2 -o 0 %s.sorted %s.bim.sorted' %(
            fn_strand,bfile,)
        cmd += ' > %s' %(fn_extract)
        execmd(cmd)
        ...
        ##
        ## check that .strand and .bim files match
        ##
        cmd = 'join -1 1 -2 2 -o 1.6,2.5,2.6 '
        cmd += ' %s.sorted %s.bim.sorted' %(fn_strand,bfile,)
        ## init awk
        cmd += " | awk '{"
        ## init if
        cmd += 'if ('
...
def count_SNPs():
    ...
    cmd = '''cat omni2.5-4_20120904_agv_gtu_sexupdated_flipped.unlifted.BED | awk 'NR%2==0{if ($1!="chr24" && $1!="chr25" && $1!="chr26") print}' | wc -l'''
    append_stats(cmd)
    ...
    for suffix in [
        ...
        'flipped',
        'build37',
        'exclmissmult','deduplicated','common',
        'renamed',
        '',
        ]:
        for i in xrange(2):
            if i==1 and suffix in ['sexupdated','build37','renamed',]:
                continue
            bfile = l_bfiles[i]
            append_stats("cat %s.bim | awk '{print $2}' | wc -l" %(bfile))
            append_stats("cat %s.bim | awk '{if($1>=1 && $1<=22) print $2}' | wc -l" %(bfile))
            append_stats("cat %s.bim | awk '{if($1==23) print $2}' | wc -l" %(bfile))
            l_bfiles[i] += '_'+suffix
    ...
    l_suffixes = ['build37','exclmissmult','deduplicated','common','',]
    l_strands = [
        'HumanOmni2.5M-b36',
    ...
    for suffix in l_suffixes:
        for i in xrange(2):
            if i == 1 and suffix == 'build37':
                continue
            strand = l_strands[i]
            append_stats("cat %s.strand | awk '{print $2}' | wc -l" %(strand))
            append_stats("cat %s.strand | awk '{if($2>=1 && $2<=22) print $2}' | wc -l" %(strand))
            append_stats('''cat %s.strand | awk '{if($2=="X") print $2}' | wc -l''' %(strand))
            l_strands[i]
...
def PLINK_split_by_population(l_bfiles):
...
def PLINK_rename(bfile):
    ...
    cmd += '--bfile %s \\\n' %(bfile,)
    cmd += '--make-bed --out %s \\\n' %(bfile_out,)
    cmd += '--update-map common.SNPs.pasted --update-name \\\n'
    cmd += '--noweb --nonfounders --allow-no-sex \\\n'
    execmd(cmd)
...
def identify_common_positions(l_bfiles):
...
def PLINK_exclude_non_intersection(l_bfiles):

    l_bfiles_out, bool_exists = check_file_out(l_bfiles,'common')
    if bool_exists == True:
        return l_bfiles_out

    ##
    ## 1) identify common positions
    ##
    identify_common_positions(l_bfiles)

    ## 2) write quad2octo.dic
    cmd = 'cat %s.%s.concatenated' %(l_bfiles[0],l_bfiles[1],)
    cmd += " | awk '{print $1,$2}'"
    cmd += ' > quad2octo.dic'
    execmd(cmd)

    ## 3) write %s.common.SNPs
    for i_bfile in xrange(len(l_bfiles)):
        cmd = 'cat %s.%s.concatenated' %(l_bfiles[0],l_bfiles[1],)
        cmd += " | awk '{print $%i}'" %(i_bfile+1)
        cmd += ' > %s.common.SNPs' %(l_bfiles[i_bfile])
        execmd(cmd)

    ## 4) exclude positions that are not shared
    for bfile in l_bfiles:
        cmd = 'plink \\\n'
        cmd += '--bfile %s \\\n' %(bfile)
        cmd += '--extract %s.common.SNPs \\\n' %(bfile)
        cmd += '--make-bed --out %s_common \\\n' %(bfile)
        cmd += '--noweb --allow-no-sex --nonfounders \\\n'
        execmd(cmd)
...
def sort_strand(fn_strand,s_print='$0',s_sort='',):

    cmd = '''cat %s | sort -k1,1 | ''' %(fn_strand,)
    cmd += '''awk '{sub(/X/,23,$2);sub(/Y/,24,$2)'''
    cmd += ''';sub(/XY/,25,$2);sub(/MT/,26,$2)'''
    cmd += ''';print %s}' %s > %s.sorted''' %(s_print,s_sort,fn_strand,)
    execmd(cmd)

    return


def PLINK_exclude_miss_and_multiple(l_bfiles,l_miss,l_multiple,):
    ...
    for i in xrange(2):
        ...
        ## exclude duplicates
        ##
        PLINK_exclude(bfile,'%s.%s.SNPs' %(miss,multiple,),bfile_out,)

    return l_bfiles_out


def PLINK_exclude_duplicates(l_bfiles):
    ...
    for bfile in l_bfiles:
        ...
        ##
        PLINK_exclude(bfile,'%s.duplicates.SNPs' %(bfile),bfile_out,)
...
def PLINK_exclude(bfile,exclude,bfile_out,):
...
def identify_duplicates(bfile,col_bim_chr,col_bim_pos,col_bim_name,):
    ...
    for line in fd_in:
        ...
        if not (
            ...
            line_prev = line
            continue
        ## preferably removal of Insertions/Deletions
        if (
...
def PLINK_update_map(bfile,):

    bfile_out = bfile+'_build37'
    if os.path.isfile('%s.bed' %(bfile_out)):
        if time.time()-os.path.getmtime('%s.bed' %(bfile_out))<hours*60*60:
            return bfile_out
    ...
    ## i.e. write build37.txt
    create_input_PLINK_update_map(bfile,)
...
def PLINK_update_sex(bfile):

    bfile_out = bfile+'_sexupdated'
    if os.path.isfile('%s.bed' %(bfile_out)):
        if time.time()-os.path.getmtime('%s.bed' %(bfile_out))<hours*60*60:
            return bfile_out
    ...
    ## clean up
    ##
    os.remove('%s_update-sex.txt' %(bfile))
...
def create_input_PLINK_update_map(bfile,):
...
def init():
    ...
    return l_bfiles, l_strands, l_miss, l_multiple
