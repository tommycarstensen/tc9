...
trimmed=$(basename $url .txt).trimmed.txt
curl -s $url \
| sed 's/ \+\t/\t/g' \
| awk -F$'\t' 'BEGIN{OFS="\t"} {
 if(NF==6) {print $0} else {print $1,$3,$4,$5,$6,$7}}' \
> $trimmed

url1=https://raw.githubusercontent.com/23andMe/yhaplo/master/input/representative.SNPs.isogg.2015tree.txt
url2=https://raw.githubusercontent.com/23andMe/yhaplo/master/input/representative.SNPs.additional.txt
join -t$'\t' \
 <(curl -s $url1 $url2 \
  | awk '{print $1,$2}' \
  | grep -v ^[C-D,F-Z][1-3] \
 ...
 <(cat $trimmed \
  | awk -F$'\t' 'BEGIN{OFS="\t"} {
   print $1,$(NF-4),int($(NF-1)),substr($NF,1,1),substr($NF,4,1)}' \
  | sort -t$'\t' -k1,1) \
...
cat $trimmed | grep -v Investigation \
| awk -F$'\t' 'BEGIN{OFS="\t"} length($6)==4&&substr($6,2,1)=="-"&&substr($6,3,1)==">"&&(substr($2,1,1)=="A"||substr($2,1,1)=="B"||substr($2,1,1)=="E"){
 print $2,$1,$5,substr($6,1,1),substr($6,4,1)}' \
| shuf | awk 'BEGIN{OFS="\t"} {cnt[$1]++; if(cnt[$1]<=5) {print $0,2}}' \
| sort -k1V,1 -k3n,3
...
cat $trimmed | grep -v Investigation | grep -v Withdrawn \
| awk -F$'\t' 'BEGIN{OFS="\t"} length($6)==4&&substr($6,2,1)=="-"&&substr($6,3,1)==">"&&(substr($2,1,1)!="A"&&substr($2,1,1)!="B"&&substr($2,1,1)!="E")&&length($2)<=4{
 print $2,$1,$5,substr($6,1,1),substr($6,4,1)}' \
| shuf | awk 'BEGIN{OFS="\t"} {cnt[$1]++; if(cnt[$1]<=3) {print $0,3}}' \
| sort -k1V,1 -k3n,3
